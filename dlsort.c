...
typedef struct node {
  int data;
  struct node *prev; /* pointer to prev element in list */
...
void bst_copy(struct bst_node *r, LLIST *n) {
  ...
  if (node == NULL)  return;
  bst_copy(node->left, n->prev);
  prt_tnode("bst_copy", r);
  prt_node("bst_copy", n);
  n->data = r->data;
  bst_copy(node->right, n->prev);
}
...
int main(int argc, char *argv[]) {
  ...
  n = p;
  for(i = 2; i < argc; i++) p = list_add(p, atoi(argv[i]));
  list_print(n);
